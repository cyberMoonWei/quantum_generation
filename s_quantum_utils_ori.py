...
# ── Circuit functional score ─────────────────────────────────────────────────────────────
def f_score(qc, _c=canon_qc, _m=is_meas):
    if qc is None or _c is None:
        return None
    try:
        return 1.0 - tvd(qc, _c, shots=SHOTS) if _m else circuit_fidelity(qc, _c)
    except Exception:
        return None
